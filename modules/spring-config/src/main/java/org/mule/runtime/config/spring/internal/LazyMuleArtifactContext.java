...
public class LazyMuleArtifactContext extends MuleArtifactContext implements LazyComponentInitializer {
  ...
  public LazyMuleArtifactContext(MuleContext muleContext, ConfigResource[] artifactConfigResources,
    ...
    try {
      muleContext.getRegistry().registerObject("_muleLazyInitService", this);
  ...
  public void initializeComponent(Location location) {
    applyLifecycle(createComponents(location, null));
  }

  @Override
  public void initializeComponents(String configFileName) {
    applyLifecycle(createComponents(null, configFileName));
  }
  ...
  public void createComponent(Location location) {
    createComponents(location, null);
  }

  private List<String> createComponents(Location location, String configFile) {
    ...
    withContextClassLoader(muleContext.getExecutionClassLoader(), () -> {
      ...
      unregisterComponents(alreadyCreatedApplicationComponents);

      ApplicationModel minimalApplicationModel;
      if (location != null) {
        minimalApplicationModel = minimalApplicationModelGenerator.getMinimalModel(location);
      } else {
        minimalApplicationModel = minimalApplicationModelGenerator.getMinimalModel(configFile);
      }
  ...
  private void unregisterComponents(List<String> beanNames) {
    if (muleContext.isStarted()) {
